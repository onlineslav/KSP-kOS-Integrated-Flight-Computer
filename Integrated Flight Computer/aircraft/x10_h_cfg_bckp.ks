...
FUNCTION BUILD_AIRCRAFT_CONFIG {
  RETURN LEXICON(
    ...
    "tailstrike_pitch_max_deg", 16.5, // deg absolute pitch cap near runway (takeoff + flare)
    ...
    "flare_gear_h_offset_m",   2.0, // m positive bias so flare logic treats gear as slightly higher
    "flare_agl",               36,   // m runway-relative main-gear height to begin flare
    ...
    "flare_roundout_end_h_m",    1.5, // m runway-relative height where roundout is fully blended
    ...
    "flare_roundout_start_h_m",      16.0, // m runway-relative main-gear final sink blend zone
    "flare_roundout_curve", 1.0,  // restore roundout shaping to avoid long float then drop
    "flare_cmd_rate_min_dps",    1.2,  // deg/s low-speed flare response
    "flare_cmd_rate_max_dps",    3.2,  // deg/s high-speed flare response
    ...
    "flare_tecs_eb_kp",          0.157,  // V*g-normalized gamma proportional gain on energy-balance error (= 0.0002 * 80 * 9.81)
