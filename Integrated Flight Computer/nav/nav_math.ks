...
FUNCTION GEO_DESTINATION {
  ...
  LOCAL BR IS SHIP:BODY:RADIUS.
  LOCAL ang IS dist_m / BR.   // angular distance (radians)
...
FUNCTION CROSS_TRACK_ERROR {
  ...
  LOCAL BR IS SHIP:BODY:RADIUS.
  LOCAL d13 IS GEO_DISTANCE(from_ll, ac_ll) / BR.        // angular dist from_ll -> ac
  LOCAL brng13 IS GEO_BEARING(from_ll, ac_ll) * CONSTANT:DEGTORAD.
  LOCAL brng12 IS GEO_BEARING(from_ll, to_ll) * CONSTANT:DEGTORAD.
  RETURN BR * ARCSIN(CLAMP(SIN(d13) * SIN(brng13 - brng12), -1, 1)) * CONSTANT:DEGTORAD.
}
...
FUNCTION ALONG_TRACK_DIST {
  PARAMETER ac_ll, from_ll, to_ll.
  LOCAL BR IS SHIP:BODY:RADIUS.
  LOCAL d13 IS GEO_DISTANCE(from_ll, ac_ll) / BR.
  LOCAL xt  IS CROSS_TRACK_ERROR(ac_ll, from_ll, to_ll) / BR.
  RETURN BR * ARCCOS(CLAMP(COS(d13) / COS(xt), -1, 1)) * CONSTANT:DEGTORAD.
}
